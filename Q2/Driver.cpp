...
int main()
{
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        std::string output_file_name = "output_2bii_" + std::to_string(values[i]).substr(0, 5) + ".dat";

        // Use Trapezoidal method with each step size
        TrapezoidalSolver* p_trapezoidal_solver_ii = new TrapezoidalSolver(*p_linear_ode, 0.0, 0.0, 1.0, values[i], output_file_name);
        p_trapezoidal_solver_ii->Solve();
        ...
        // Deallocate memory
        delete p_trapezoidal_solver_ii;
    }
